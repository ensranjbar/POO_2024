...
public class Stanza {
	...
	//	Stanza stanza2 = this;
		for (int i = 0; i < this.direzioni.length; i++)
			if ((direzione != null)  && (direzione.equals(this.direzioni[i]))) {
				this.stanzeAdiacenti[i] = stanza;
				aggiornato = true;
			}
		if (!aggiornato)
			if (direzione!=null &&  this.numeroStanzeAdiacenti < NUMERO_MASSIMO_DIREZIONI) {
	...
	public boolean hasAttrezzo(String nomeAttrezzo) {
		...
		trovato = false;
		for (Attrezzo attrezzo : this.attrezzi) {
			if (attrezzo != null && nomeAttrezzo != null && attrezzo.getNome().equals(nomeAttrezzo))
				trovato = true;
		}
		return trovato;
	}
	...
	public boolean removeAttrezzo(String attrezzo) {
		if (attrezzo==null || this.numeroAttrezzi == 0 || !this.hasAttrezzo(attrezzo))
			return false;
		Attrezzo attrezzoDaRemove = this.getAttrezzo(attrezzo); // rappresentiamo un attrezzo da remove che presente in
																// stanza
		for (int i = 0; i < this.numeroAttrezzi; i++) {
			if (this.attrezzi[i] != null && this.attrezzi[i].equals(attrezzoDaRemove) && i<numeroAttrezzi-1) {
